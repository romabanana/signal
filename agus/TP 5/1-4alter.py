...
# gráficos
idx_validos = w <= np.pi/2 # limita la visualizacion dentro del rango de Nyquist
plt.plot(freq[idx_validos],
         np.abs(H1[idx_validos]),
         label='Continuo')

plt.plot(freq[idx_validos],
         np.abs(He[idx_validos]),
         label='Euler')

plt.plot(freq[idx_validos],
         np.abs(Hb[idx_validos]),
         label='Bilineal')
