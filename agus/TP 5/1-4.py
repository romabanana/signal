...
freq = frecuencias(5000, 5000) * 2 * np.pi # frecuencia angular
idx = freq >= 0
freq = freq[idx]
...
#T = 1 / (50 * fc) # aumentar la frecuencia de muestreo mejora la aproximacion
w = freq * T

idx = w <= np.pi # me quedo solo con las frecuencias dentro del rango de Nyquist
w = w[idx]
freq = freq[idx]
H1 = H1[idx]
...
plt.plot(freq, np.abs(He), label='Euler')
plt.plot(freq, np.abs(Hb), label='Bilineal') # la bilineal se aproxima mejor al continuo
